...
async function carregarPosts(): Promise<Post[]> {
...
async function atualizarExibicaoPostsAsync() {
  ...
  if (!containerPost)
    throw new Error(
      'Não é possível iniciar a aplicação sem o container de post.'
    );
  ...
  const htmlPost = posts.map((post) => {
    ...
    return `
          <div class="post">
              <h3 class="titulo-post">${post.title}${
      ehFavorito ? ' (❤️)' : ''
    }</h3>
...
function carregarFavoritosExistentes() {
  ...
  let listaFavoritos: string[] = favoritos ? JSON.parse(favoritos) : [];
  return listaFavoritos;
}

function salvarFavorito(tituloPost: string) {
...
function removerFavorito(tituloPost: string) {
  let listaFavoritos = carregarFavoritosExistentes();
